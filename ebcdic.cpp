...
std::string ebcdic2ascii( const std::string&input )
{
    std::string r;
    ...
    char *pAscii = new char[input.length()*2+1];
    memset(pAscii, 0x00, input.length()*2+1);
        
    ebcdic2ascii(pAscii, input.c_str(), input.length());
    r = pAscii;
...
std::string ascii2ebcdic( const std::string&input )
{
    std::string r;
    ...
    char *pEbcDic = new char[input.length()*2+1];
    memset(pEbcDic, 0x00, input.length()*2+1);

    ascii2ebcdic(pEbcDic, input.c_str(), input.length());
    r = pEbcDic;
